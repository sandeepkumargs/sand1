...
async function calculateAnalytics(){
    try{
        ...
        const quadAggregates = await getQuadrantAnalytics(activeEmployees);
        const analyticsObj = {
            "a_id":"1",
            "total_employees":activeEmployees.length,
            "total_admins":(empScoresDb.length - activeEmployees.length),
...
async function getQuadrantAnalytics(empScoresDb){
    try {
        let quadrantMap = await getQuadrantMap();
        quadrantMap.Quadrants.forEach(quadrant=>{
            let total_score = 0;
            let my_score = 0;
            for(var i=0;i<empScoresDb.length;i++){
                const empQuad = empScoresDb[i].skills_master.quadrants.find(quad=>quad.Name===quadrant.Name);
                if(empQuad){
                    my_score += empQuad.Score;
                    total_score+=empQuad.Total;
                }
            } 
            quadrant.Score = my_score;
            quadrant.Total = total_score;
        });
